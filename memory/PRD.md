...
- **Problema:** `Cannot read properties of undefined (reading 'replace')` - campo `user.role` não existe no modelo DARPE
- **Solução:** Atualizado para usar `funcoes_darpe` (array) com tratamento de valores undefined/null
- **Status:** ✅ CORRIGIDO

### 2. Bug de Persistência de Sessão
- **Problema:** Login bem-sucedido mas usuário redirecionado de volta para tela de login
- **Solução:** 
  - Refatoração completa do `AuthContext.js` com gerenciamento de estado melhorado
  - Adição de flag `isInitialized` para controlar renderização
  - Correção da ordem de atualização de estado no login
  - Atualização do `ProtectedRoute.js` para verificar token corretamente
  - Simplificação do redirecionamento no `LoginPage.js`
- **Status:** ✅ CORRIGIDO
...
- [ ] Atualização da Credencial Digital (WhatsApp, funções DARPE, QR Code)
...
├── routes_auth_public.py  # Autenticação e rotas públicas
├── routes_admin.py    # Rotas administrativas (users, units)
├── routes_features.py # Funcionalidades (attendance, service, etc.)
...
- **users:** whatsapp (unique), nome_completo, funcoes_darpe[], status, ultimo_atendimento
